...
if ($_POST && isset($_POST['titre'])) {
    ...
    if ($article) {
        query('UPDATE articles SET titre = ?, slug = ?, chapeau = ?, contenu = ?, categorie_id = ?, statut = ?, updated_at = NOW() WHERE id = ?',
            [$titre, $slug, $chapeau, $contenu, $categorie_id, $statut, $article['id']]);
        setFlash('success', 'Article mis à jour');
    } else {
        query('INSERT INTO articles (titre, slug, chapeau, contenu, categorie_id, statut, date_publication) VALUES (?, ?, ?, ?, ?, ?, NOW())',
