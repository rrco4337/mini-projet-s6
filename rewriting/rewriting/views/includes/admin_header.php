 ...
 */
$user = Auth::user();
$adminHeaderMode = $adminHeaderMode ?? 'full';
$adminSearchPlaceholder = $adminSearchPlaceholder ?? 'Rechercher un article, une categorie...';
$adminHeaderButtons = $adminHeaderButtons ?? [];
?>

<?php if ($adminHeaderMode === 'simple'): ?>
<header class="sticky top-0 z-20 border-b border-slate-200/70 bg-white/85 backdrop-blur">
  <div class="mx-auto flex max-w-7xl items-center justify-between gap-4 px-4 py-4 sm:px-6 lg:px-8">
    <div class="relative w-full max-w-xl">
      <input type="search" placeholder="<?= e($adminSearchPlaceholder) ?>" class="h-11 w-full rounded-xl border border-slate-200 bg-slate-50 px-4 text-sm outline-none transition focus:border-blue-300 focus:bg-white" />
    </div>
    <div class="flex items-center gap-3">
      <?php foreach ($adminHeaderButtons as $button): ?>
        <a
          href="<?= e($button['href']) ?>"
          <?php if (!empty($button['target'])): ?>target="<?= e($button['target']) ?>"<?php endif; ?>
          class="<?= e($button['class']) ?>"
        ><?= e($button['label']) ?></a>
      <?php endforeach; ?>
    </div>
  </div>
...
<header class="sticky top-0 z-20 border-b border-slate-200/70 bg-white/85 backdrop-blur">
  <div class="mx-auto flex max-w-7xl items-center justify-between gap-4 px-4 py-4 sm:px-6 lg:px-8">
    <div class="relative w-full max-w-xl">
      ...
      <input type="search" placeholder="<?= e($adminSearchPlaceholder) ?>" class="h-11 w-full rounded-xl border border-slate-200 bg-slate-50 pl-12 pr-4 text-sm outline-none ring-0 transition placeholder:text-slate-400 focus:border-blue-300 focus:bg-white focus:shadow-soft" />
    </div>
    <div class="flex items-center gap-3">
