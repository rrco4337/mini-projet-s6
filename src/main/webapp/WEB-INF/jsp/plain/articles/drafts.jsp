...
<div class="flex min-h-screen">
  <aside class="fixed inset-y-0 left-0 z-30 hidden w-72 border-r border-slate-200 bg-white/95 px-6 py-7 backdrop-blur xl:block">
    ...
    <nav class="mt-10 space-y-1">
      ...
      <a href="/noframework/admin/articles/drafts" class="rounded-xl bg-blue-50 px-4 py-3 font-semibold text-blue-700 block">Brouillons</a>
      <a href="/noframework/admin/articles/archives" class="rounded-xl px-4 py-3 text-slate-600 transition hover:bg-slate-100 block">Archives</a>
  ...
  <div class="w-full xl:ml-72">
    <header class="sticky top-0 z-20 border-b border-slate-200/70 bg-white/85 backdrop-blur">
      <div class="mx-auto flex max-w-7xl items-center justify-between gap-4 px-4 py-4 sm:px-6 lg:px-8">
        <div class="relative w-full max-w-xl"><input type="search" placeholder="Rechercher dans les brouillons..." class="h-11 w-full rounded-xl border border-slate-200 bg-slate-50 px-4 text-sm outline-none transition focus:border-blue-300 focus:bg-white" /></div>
        <div class="flex items-center gap-3">
          <a href="/noframework/admin/articles/archives" class="rounded-xl border border-slate-200 bg-white px-4 py-2 text-sm font-medium text-slate-700 transition hover:bg-slate-50">Archives</a>
    ...
    <main class="mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:px-8">
      <section class="mb-6 flex flex-wrap items-center justify-between gap-4"><div><h1 class="text-3xl font-bold tracking-tight text-slate-900">Tous les brouillons</h1><p class="mt-2 text-sm text-slate-600">Retrouvez uniquement les articles en cours de redaction.</p></div><a href="/noframework/admin/articles/new" class="inline-flex items-center gap-2 rounded-xl bg-slate-900 px-5 py-3 text-sm font-semibold text-white transition hover:bg-slate-700">Ajouter un article</a></section>
        ...
        <div class="overflow-x-auto">
          <table class="min-w-full text-sm">
            ...
            <% if (articles == null || articles.isEmpty()) { %>
              <tr><td colspan="5" class="px-6 py-14 text-center text-sm text-slate-500">Aucun brouillon disponible.</td></tr>
            <% } else {
              ...
              <tr class="<%= i % 2 == 0 ? "bg-white" : "bg-slate-50" %> transition hover:bg-blue-50/60">
                ...
                <td class="px-5 py-4 text-right">
                  <div class="inline-flex items-center gap-2">
                    ...
                    <form action="/noframework/admin/articles/archive" method="post" class="inline" onsubmit="return confirm('Archiver ce brouillon ?');"><input type="hidden" name="id" value="<%= article.getId() %>" /><button type="submit" class="rounded-lg border border-amber-200 bg-amber-50 px-3 py-1.5 text-xs font-semibold text-amber-700 transition hover:bg-amber-100">Archiver</button></form>
                    <form action="/noframework/admin/articles/delete" method="post" class="inline" onsubmit="return confirm('Supprimer ce brouillon ?');"><input type="hidden" name="id" value="<%= article.getId() %>" /><button type="submit" class="rounded-lg border border-rose-200 bg-rose-50 px-3 py-1.5 text-xs font-semibold text-rose-700 transition hover:bg-rose-100">Supprimer</button></form>
                  </div>
                </td>
              </tr>
            <%   }
