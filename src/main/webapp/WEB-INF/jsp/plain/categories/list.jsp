...
<%@ page import="com.miniprojets6.plain.category.PlainCategoryListRow" %>
<!DOCTYPE html>
<html lang="fr">
<head>
  <meta charset="UTF-8" />
...
<%
  List<PlainCategoryListRow> categories = (List<PlainCategoryListRow>) request.getAttribute("categories");
  ...
  <div class="w-full xl:ml-72">
    ...
    <main class="mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:px-8">
      <section class="mb-6 flex flex-wrap items-center justify-between gap-4">
        ...
        <a href="/noframework/admin/categories/new" class="rounded-xl bg-slate-900 px-5 py-3 text-sm font-semibold text-white transition hover:bg-slate-700">Nouvelle categorie</a>
      </section>
          ...
          <table class="min-w-full text-sm">
            ...
            <tbody class="divide-y divide-slate-200">
              <% if (categories == null || categories.isEmpty()) { %>
                <tr><td colspan="5" class="px-6 py-14 text-center text-sm text-slate-500">Aucune categorie disponible.</td></tr>
              <% } else {
                   for (int i = 0; i < categories.size(); i++) {
                     PlainCategoryListRow cat = categories.get(i); %>
                <tr class="<%= i % 2 == 0 ? "bg-white" : "bg-slate-50" %> transition hover:bg-blue-50/60">
                  <td class="px-5 py-4 font-semibold text-slate-700"><%= cat.getId() %></td>
                  <td class="px-5 py-4 font-semibold text-slate-900"><%= cat.getNom() %></td>
                  <td class="px-5 py-4 text-slate-600"><%= cat.getSlug() %></td>
                  <td class="px-5 py-4 text-slate-600"><%= cat.getDescription() == null ? "" : cat.getDescription() %></td>
                  <td class="px-5 py-4 text-right">
                    <div class="inline-flex items-center gap-2">
                      <a href="/noframework/admin/categories/edit?id=<%= cat.getId() %>" class="rounded-lg border border-blue-200 bg-blue-50 px-3 py-1.5 text-xs font-semibold text-blue-700 transition hover:bg-blue-100">Modifier</a>
                      <form action="/noframework/admin/categories/delete" method="post" class="inline" onsubmit="return confirm('Supprimer cette categorie ?');">
